...
import cursor from '../../../assests/images/cursor3.cur';
...
const DataBox = ({ title, qty, qtyPercentage, profit }) => (
  ...
  <Box w={['full', '20%']} boxShadow={'-2px 0 10px rgba(107, 70, 193 ,0.5)'} p={8} borderRadius={'lg'} >
    <Text children={title} />
    <HStack spacing={6}>
      <Text size={'2xl'} fontWeight={'bold'} children={`${qty}`} />
...
const DashBoard = () => {
  ...
  const {loading , usersCount,
    ...
    subscriptionProfit,
    viewsProfit,
    userProfit ,stats} = useSelector(state=>state.admin)
  useEffect(() => {
    ...
    >
      {
        ...
        <Box m={[0, 16]} borderRadius={'lg'} mt={[4, 16]} p={['0', '16']} boxShadow={'-2px 0 10px rgba(107, 70, 193 ,0.5)'}  >
